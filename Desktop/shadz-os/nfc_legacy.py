"""NFC legacy system and internal utility routes.

Extracted from main.py in Phase 4H. Contains:
  - X-API-Key authentication (require_api_key) used by legacy routes
  - GET  /status           — server monitoring (X-API-Key protected)
  - POST /run-command      — safe shell runner (X-API-Key protected)
  - POST /nfc              — create NFC record (X-API-Key protected)
  - GET  /nfc/{tag_id}     — read NFC record (X-API-Key protected)
  - PUT  /nfc/{tag_id}     — update NFC record (X-API-Key protected)
  - GET  /r/{tag_id}       — NFC public redirect (no auth)
  - PATCH /admin/nfc       — legacy NFC admin update (HTTP Basic via admin_router)

Call register_nfc_routes(app) in main.py before the /{slug} catch-all.
Call register_nfc_admin_routes(admin_router) in main.py before app.include_router(admin_router).
"""
import os
import time
import platform
...
def register_nfc_routes(app) -> None:
    """Register NFC and legacy utility routes onto the FastAPI app.

    Routes registered here:
      GET  /status        — server monitoring (X-API-Key)
      POST /run-command   — safe shell runner (X-API-Key)
      POST /nfc           — create NFC record (X-API-Key)
      GET  /nfc/{tag_id}  — read NFC record (X-API-Key)
      PUT  /nfc/{tag_id}  — update NFC record (X-API-Key)
      GET  /r/{tag_id}    — NFC public redirect (no auth)

    Call this in main.py before the /{slug} catch-all route.
    """
...
def register_nfc_admin_routes(admin_router) -> None:
    """Register the legacy NFC admin route onto the shared admin_router.
