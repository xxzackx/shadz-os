...
class MediaAssetSearchFilterTests(unittest.TestCase):
    ...
    def test_load_storage_populates_cache_for_local_filtering(self):
        self.assertIsNotNone(self.load_fn, "loadStorage() body not found")
        self.assertIn("_allAssets = data;", self.load_fn)
        self.assertIn("_allAssets = [];", self.load_fn)  # error/empty responses clear it
