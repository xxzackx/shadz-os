...
- `GET /admin/pages/{page_id}` JSON read endpoint — to support Edit Page pre-fill in admin UI; not required for current v1 — confirmed still deferred, not part of Phase 4I or 4K
- **Phase 4Ka — Check Slug Page Optimisation** — not started. Deferred from Phase 4K test 6: the "Manage Page Attachment" shortcut's destination section (`pageAttachSection`) opens at the bottom of the existing Check Slug results page instead of scrolling into view, and on desktop the panel width/layout changes unexpectedly when opened this way (mobile does not show the width issue, but the bottom-opening placement remains)
- Future Page Engine UX improvements generally (beyond 3E polish, 4K polish, the read-endpoint gap, and the 4Ka navigation/layout gap above) — not scoped or started
