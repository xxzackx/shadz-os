...
- Activation Engine v1 Phase A4U — URL Content Setup (`67557d3`, amended by the runtime-fix commit below) — **implementation and focused tests complete on `master`; not yet deployed to production.** For an eligible unactivated `url` slug only, extends the Phase A3 Telegram session with two more in-memory states (`awaiting_activation_url`, `awaiting_activation_url_confirmation`) instead of parking at the generic `awaiting_activation_setup` placeholder — `media` slugs are unaffected and still land at that placeholder with no follow-up message. Immediately after the Phase A3 access-code message, `bot_runtime._handle_activation_callback` sends a prompt for the destination URL. `bot_runtime._handle_message` validates the reply with the same `http://`/`https://` prefix check and `_is_blocked_destination_url` SHADZ/internal/localhost/loopback guard already used by the existing T1B self-service flow; an invalid or blocked URL stays retryable in the same state with no database change. A valid URL is shown back to the customer as the **validated, trimmed** destination (`text.strip()` only — no scheme/host/path canonicalization; never described as "normalized" in code, docs, or tests) alongside a **"Confirm" / "Change URL" inline keyboard** (narrowly-scoped `a4uconfirm_<token>` / `a4uchange_<token>` callback_data, handled by new `bot_runtime._handle_a4u_confirmation_callback`, routed by the webhook on prefix before the existing Phase A2/A3 `activate_` dispatch) — this is the primary confirmation path; typed `YES`/`NO` is kept only as a compatibility fallback in `_handle_message`. Change URL / typed `NO` returns to URL input, clearing both `pending_url` and any `confirmed_destination_url`, without any database or lifecycle change. Confirm / typed `YES` stores the confirmed URL only in the in-memory session (`session["confirmed_destination_url"]`) and returns the session to the `awaiting_activation_setup` placeholder for Phase A5 to pick up — it is never written to `RedirectLink.destination_url` or any other live runtime. Both the callback and typed paths are **state-based idempotent**: a repeat Confirm/YES after the session already advanced past confirmation (same token, already in the placeholder state with a stored `confirmed_destination_url`) is a safe no-op — no DB write, no session reset to `awaiting_code`, no loss of the confirmed value — proven both under Telegram's existing `update_id` dedup and under a genuinely different `update_id`, since the safety comes from the session's current state, not the dedup cache. A repeat Change URL tap once already back at URL input is likewise a no-op. Every message/callback in the pre-confirmation states re-validates the token/record/content_type from scratch via the existing `bot_runtime._lookup_unactivated_record` plus a `content_type == "url"` check, so an archived, already-activated, wrong-type, or otherwise stale/invalid session fails closed (clears the session, generic invalid-link response) the same way Phase A2/A3 already do — this guard is unchanged by the amendment. Phase A4U never sets `ActivationRecord.owner_client_id`/`activation_status`/`activated_at`, never assigns a `BotClientSlug`, and never writes to `RedirectLink.destination_url` — those remain Phase A5. `tests/test_activation_engine_phase_a4u.py` (36 stdlib-`unittest` tests, isolated in-memory SQLite plus a `TestClient`-driven webhook-integration class, `bot_runtime._send_message`/`_answer_callback_query` mocked — no network calls) covers: eligible url slug entering URL-input state; media slug never entering any A4U state; the confirmation prompt carrying both inline actions with correctly-scoped callback_data; valid URL reaching confirmation with the validated/trimmed value shown; invalid-format, `shadz.io`, `localhost`, and loopback URLs staying retryable with no DB change; Confirm (button and typed YES) storing the confirmed URL session-only without touching `ActivationRecord`/`RedirectLink`/`BotClientSlug`; Change URL (button and typed NO) returning to URL input and clearing both pending and confirmed values; an unrecognised typed reply staying in the confirmation state; archived/already-activated/wrong-type/invalid-session failing closed for both the typed and callback paths; duplicate Confirm delivery (same `update_id`, a different `update_id`, and a stray typed duplicate) leaving the session in the placeholder state with the confirmed URL intact and creating no duplicate `BotClient`/`BotClientSlug` rows; and the pre-existing Phase A2/A3 `activate_` callback dispatch continuing to work unchanged through the webhook route. `tests/test_activation_engine_phase_a2.py` and `tests/test_activation_engine_phase_a3.py` are unchanged by this amendment (their three prior updates, from the original A4U commit, still stand). Verification: 36/36 focused tests passed, 112/112 A2+A3 regression tests passed, 229/229 full suite passed, `py_compile` clean, `git diff --check` clean. **Not deployed** — no VPS access was performed as part of this phase; production `shadz.service` still runs the Phase A3 build (`acb8690`) until a separate approved deploy step.
...
- **Activation Engine v1 — locked roadmap. Phase A1 (`54e0801`/`745d304`, closed 2026-07-24, `activation_records` data foundation), Phase A2 (`9ec0458`+`e713480`, closed, deployed and production-tested — First-Scan Telegram Entry plus the admin provisioning correction), and Phase A3 (`acb8690`, closed, deployed and production-tested — Bot Client Creation and Access Code) are all complete and closed — see Completed milestones. Remaining phases:**
  - **Phase A4U — URL Content Setup:** implementation and focused tests complete on `master` (see Completed milestones) — **not yet deployed to production**; no live/VPS verification has been performed
  - **Phase A4M — Media Content Setup:** media slug asks for media upload; validate and confirm the media — not started
  - **Phase A5 — Activation Completion and Public Runtime:** atomically save the final content; set `owner_client_id`; set `activated_at`; set `activation_status` to `activated`; future scans use normal URL redirect or media rendering; prevent repeat activation
  - **Phase A6 — Regression, Live Testing and Production Closure:** automated regression testing; production live testing; confirm no second activation; production closure and documentation closure
