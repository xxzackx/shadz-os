...
- **Bot Engine — remaining work (not yet implemented, unnamed future phase):**
  ...
  - Regenerate access code and rename/deactivate bot client are not yet exposed in the admin UI (backend routes exist: `POST /admin/bot/clients/{id}/regenerate-code`, `PATCH /admin/bot/clients/{id}`) — deferred from T1D and T1E to keep each change surgical
