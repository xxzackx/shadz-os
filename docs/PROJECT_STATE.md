...
- **Activation Engine v1 — locked roadmap. Phase A1 (`54e0801`/`745d304`, closed 2026-07-24, `activation_records` data foundation), Phase A2 (`9ec0458`+`e713480`, closed, deployed and production-tested — First-Scan Telegram Entry plus the admin provisioning correction), Phase A3 (`acb8690`, closed, deployed and production-tested — Bot Client Creation and Access Code), Phase A4U (`67557d3`/`7f2ed23`/`fd70dd2`/`f0c8ffe`, closed, deployed and production-tested — URL Content Setup), and Phase A4M (`08c0ac0`, closed, deployed and production-tested — Media Content Setup, zero-persistence) are all complete and closed — see Completed milestones. Remaining phases, in locked order:**
  - **Phase A5 — Activation Finalization (next planned phase):** atomically save the final content; set `owner_client_id`; set `activated_at`; set `activation_status` to `activated`; future scans use normal URL redirect or media rendering; prevent repeat activation — remains solely responsible for activation finalization and any live redirect/media ownership change; Phase A4U/A4M never perform this — not started
  - **Phase A6 — Regression, Live Testing and Production Closure:** automated regression testing; production live testing; confirm no second activation; production closure and documentation closure for the complete Activation Engine v1
