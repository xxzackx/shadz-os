...
- **Activation Engine v1 — remaining work (Phase A1 `54e0801`/`745d304` closed 2026-07-24 delivered the `activation_records` data foundation; Phase A2 completed locally, not yet pushed/deployed, delivered the public Activation Gateway redirect and Telegram `/start` activation-entry recognition — see Completed milestones):**
  - Activation routing / pre-activation redirect behavior for the NFC chip URL (`https://shadz.io/{slug}`) — done (Phase A2): unactivated `url`/`media` slugs redirect into the Telegram activation deep link
  - Telegram bot activation entry recognition (`/start activate_<token>` → Phase A2 entry message) — done (Phase A2)
  - "Activate Now" button next-step action — the callback itself is handled in Phase A2 (acknowledged, revalidated, generic boundary response); what happens *after* that boundary (the actual next activation step) is not started, deferred to a later phase
  - Token consumption / activation confirmation flow — not started
