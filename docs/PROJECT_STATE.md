...
**Admin UI version:** Phase T1D `67fad4e` — Telegram Bot Clients admin UI (deployed 2026-07-04). Previous: Phase 3D `357a85e` Page Engine JSON helper guidance. Bot Engine Phase T1 (`28559a3`) added backend routes only. Phase 3B `a57fff7` Page Engine admin UI. Phase 3C (`165c0d3`) added public page rendering — no admin UI change. Phase 1B `1d11005` media asset rename. Phase C `45d2656` CSV export.
...
- **Bot Engine — remaining work (not yet implemented, unnamed future phase):**
  - Optional cleanup of Phase T1B live test data (test bot client + slug `t1b-test-ioayej`) — still outstanding; not touched during T1C or T1D (T1C's own temporary test client/assignment were cleaned up, see T1C changelog entry, but this is separate T1B-era test data)
  - Regenerate access code and rename/deactivate bot client are not yet exposed in the admin UI (backend routes exist: `POST /admin/bot/clients/{id}/regenerate-code`, `PATCH /admin/bot/clients/{id}`) — deferred from T1D to keep the change surgical
