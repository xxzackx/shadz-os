...
- Activation Engine v1 Phase A4U — URL Content Setup (`67557d3`) — **implementation and focused tests complete on `master`; not yet deployed to production.** For an eligible unactivated `url` slug only, extends the Phase A3 Telegram session with two more in-memory states (`awaiting_activation_url`, `awaiting_activation_url_confirmation`) instead of parking at the generic `awaiting_activation_setup` placeholder — `media` slugs are unaffected and still land at that placeholder with no follow-up message. Immediately after the Phase A3 access-code message, `bot_runtime._handle_activation_callback` sends a prompt for the destination URL. `bot_runtime._handle_message` validates the reply with the same `http://`/`https://` prefix check and `_is_blocked_destination_url` SHADZ/internal/localhost/loopback guard already used by the existing T1B self-service flow; an invalid or blocked URL stays retryable in the same state with no database change. A valid URL is shown back to the customer (normalized via `.strip()`) with an explicit YES/NO confirmation prompt; NO/retry/change returns to URL input (clearing the pending value) without any database or lifecycle change; YES stores the confirmed URL only in the in-memory session (`session["confirmed_destination_url"]`) and returns the session to the `awaiting_activation_setup` placeholder for Phase A5 to pick up — it is never written to `RedirectLink.destination_url` or any other live runtime. Every message in either new state re-validates the token/record/content_type from scratch via the existing `bot_runtime._lookup_unactivated_record` plus a `content_type == "url"` check, so an archived, already-activated, wrong-type, or otherwise stale/invalid session fails closed (clears the session, generic invalid-link message) the same way Phase A2/A3 already do. Phase A4U never sets `ActivationRecord.owner_client_id`/`activation_status`/`activated_at`, never assigns a `BotClientSlug`, and never writes to `RedirectLink.destination_url` — those remain Phase A5. New `tests/test_activation_engine_phase_a4u.py` (21 stdlib-`unittest` tests, isolated in-memory SQLite, `bot_runtime._send_message`/`_answer_callback_query` mocked — no network calls) covers: eligible url slug entering URL-input state; media slug never entering any A4U state; valid URL reaching confirmation with the normalized value shown; invalid-format, `shadz.io`, `localhost`, and loopback URLs all staying retryable with no DB change; YES storing the confirmed URL session-only without touching `ActivationRecord`/`RedirectLink`/`BotClientSlug`; NO returning to URL input; an unrecognised confirmation reply staying in the confirmation state; archived/already-activated/wrong-type/invalid-session fail-closed in both new states; and repeated valid submissions / a stray repeated YES creating no duplicate `BotClient` or `BotClientSlug` rows. `tests/test_activation_engine_phase_a2.py` and `tests/test_activation_engine_phase_a3.py` updated in three places (two `assert_awaited_once_with` calls, one session-state assertion) to reflect that a url slug's Phase A3 callback now sends a second message and continues into `awaiting_activation_url` instead of `awaiting_activation_setup`. Verification: 21/21 focused tests passed, 214/214 full suite passed, `py_compile` clean, `git diff --check` clean. **Not deployed** — no VPS access was performed as part of this phase; production `shadz.service` still runs the Phase A3 build (`acb8690`) until a separate approved deploy step.
