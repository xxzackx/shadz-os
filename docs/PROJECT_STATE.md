...
- **Activation Engine v1 — locked roadmap. Phase A1 (`54e0801`/`745d304`, closed 2026-07-24, `activation_records` data foundation), Phase A2 (`9ec0458`+`e713480`, closed, deployed and production-tested — First-Scan Telegram Entry plus the admin provisioning correction), Phase A3 (`acb8690`, closed, deployed and production-tested — Bot Client Creation and Access Code), and Phase A4U (`67557d3`/`7f2ed23`/`fd70dd2`/`f0c8ffe`, closed, deployed and production-tested — URL Content Setup) are all complete and closed — see Completed milestones. Remaining phases, in locked order:**
  - **Phase A4M — Media Content Setup (next planned phase):** media slug asks for media upload; validate and confirm the media — not started
  - **Phase A5 — Activation Completion and Public Runtime:** atomically save the final content; set `owner_client_id`; set `activated_at`; set `activation_status` to `activated`; future scans use normal URL redirect or media rendering; prevent repeat activation — remains solely responsible for activation finalization and any live redirect/media ownership change; Phase A4U/A4M never perform this
