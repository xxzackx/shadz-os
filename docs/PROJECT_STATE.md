...
- **Activation Engine v1 — locked roadmap. Phase A1 (`54e0801`/`745d304`, closed 2026-07-24, `activation_records` data foundation) and Phase A2 (`9ec0458`+`e713480`, closed, deployed and production-tested — First-Scan Telegram Entry plus the admin provisioning correction) are both complete and closed — see Completed milestones. Remaining phases (not started):**
  - **Phase A3 — Bot Client Creation and Access Code:** create or assign the Bot Client; generate the unique access code; connect the activation flow to the customer; branch automatically by the slug's existing content type; do not collect URL or media content yet
