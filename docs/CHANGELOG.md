...
**Scope:** The existing Bot Client management surface in `static/admin.html` was turned into a Bot Client Control Centre — assigned-slug activation-state visibility, client identity + "Last Activity", client-side search/filter, lifecycle-action UX/safety polish, and a client-side summary strip. Backend work was limited to additive read-only response enrichment on schemas already served by `GET /admin/bot/clients`. **No new routes. No database schema/model/migration. No change to Bot Client lifecycle, slug-assignment, Activation Engine, or Telegram runtime semantics. No change to NFC / public redirect behavior.**

**Final runtime commit:** `3ea7ee8` — `fix(admin): align bot client reactivate label`

- **UI3G-A — Control Centre foundation** (no implementation commit) — the pre-existing single-file "Telegram Bot Clients" section: create + list (`POST`/`GET /admin/bot/clients`), assign / unassign slug, deactivate / reactivate / delete (`PATCH`/`DELETE /admin/bot/clients/{id}`), regenerate access code, and bulk test-data cleanup (`POST /admin/bot/clients/bulk-delete` + per-card selection). This surface and its T1D–T1K backend contract were the foundation for UI3G-B onward.

- **UI3G-B — Assigned Slug Activation State** (`003db22`, `feat(admin): show bot client slug activation state`) — visibility/enrichment, not a new activation engine. `AssignedSlugOut` additively gained `activation_status`, `activation_owner_client_id`, `activation_state`. `bot_admin._derive_activation_state(client, record)` classifies each assigned slug with the same wording as the UI3D slug-search panel: `Legacy` (no `ActivationRecord` — never "unactivated"), `Awaiting Telegram Login`, `Activation Sync Required`, `Ownership Conflict`, `Activated`. `_build_assigned_slugs` OUTER-joins `ActivationRecord` (one query, no N+1) so Legacy assigned slugs stay in the response; the `GET` read path never reconciles and never calls `_reconcile_one_assigned_slug`. Each assigned-slug row on the card shows a compact state badge. Existing `ActivationRecord` semantics unchanged.

- **UI3G-C — Client Identity + Activity Visibility** (`c7d0bdf`, `feat(admin): show bot client identity and activity`) — read-only. The Bot Client card gained a **Telegram User ID** row and a **Created** row (Telegram identity fields display-only; display name still shown once as the primary Client name, per UI3D-C1 Polish). `AssignedSlugOut.activated_at` added additively from the `ActivationRecord` already joined in UI3G-B (no extra query). `BotClientOut.last_activity_at` / `last_activity_source` are derived **server-side** from existing timestamps only — "most recent meaningful activity", tie-break order **Slug activated → Slug assigned → Client updated → Client created** (`Client updated` counts only when `updated_at` post-dates `created_at`; `Client created` is the always-present fallback). No new analytics/event system.

- **UI3G-C.1 — UTC Timestamp Preservation hotfix** (`01157f9`, `fix(admin): preserve utc bot client timestamps`) — SQLite returns `DateTime(timezone=True)` values naive, so `bot_admin`'s response datetimes serialized without an offset and the browser's `new Date(...)` misread stored UTC as local time (Bot Client 13 rendered ~7h early at UTC+7). Fix: `_as_utc` made defensive (naive → UTC-tagged; already-aware / `None` pass through unchanged) and attached as `field_validator(..., mode="before")` on `created_at` / `updated_at` / `last_activity_at` (`BotClientOut`) and `assigned_at` / `activated_at` (`AssignedSlugOut`), mirroring the established `link_admin._as_utc` convention. **Serialization only** — no DB value changed, no schema change, no timezone-architecture change, Last Activity selection and labels unchanged.

- **UI3G-D — Search + Filter** (in `92627ce`, `feat(admin): complete UI3G bot client control centre`) — client-side-first. `GET /admin/bot/clients` remains the sole source dataset; `loadBotClients()` caches the full fetch into `_allBotClients`; `renderBotClients()` filters that cache with **no additional API request**. Search matches (case-insensitively) client name, Telegram username, Telegram User ID, and any assigned slug (`assigned_slugs[].slug`). Status filter: All clients / Active / Inactive / Telegram Linked / Not Linked. Search and filter combine; "no bot clients yet" and "no match" are distinct empty states; bulk selection is cleared on every re-render so a filtered-out card can never remain silently selected. No new backend search/filter endpoint.

- **UI3G-E — Lifecycle Actions Polish** (in `92627ce`; label hotfix `3ea7ee8`, `fix(admin): align bot client reactivate label`) — Regenerate Access Code / Deactivate / Reactivate / Delete / Unassign slug got state-aware, stronger confirmation copy plus duplicate-submit protection via the frontend in-flight guard `_botActionBusy` (reset in `finally`); every successful action reloads authoritative server state via `loadBotClients()`. Regenerate states the current code becomes invalid immediately and is unrecoverable; Deactivate/Reactivate states assigned slugs are preserved (existing backend semantics); Delete states Bot Client access + assignments are removed but the underlying SHADZ slug/content is **not** deleted or archived; Unassign names the slug and states the slug/content itself is **not** deleted or archived. Final terminology: **Active client → `Deactivate`, Inactive client → `Reactivate`** — the inactive button label was aligned to the already-present `Reactivate` / `reactivated` confirmation/toast copy in `3ea7ee8`. No lifecycle backend semantics changed.

- **UI3G-F — Control Centre Summary + Final Polish** (in `92627ce`) — a compact summary strip (`#bot-summary`, `renderBotSummary()`) showing **Total Clients / Active / Inactive / Telegram Linked / Not Linked**, all derived client-side from the **full `_allBotClients` dataset** — never only the current search/filter result. No new summary/count endpoint; styling stays within Admin UI v0.3 tokens. Final polish limited to the Bot Client Control Centre (search/filter row, summary strip, spacing, distinct empty/no-match states); no broad Admin Panel redesign.

- **UI3G-G — Regression + Production Verification** — completed successfully in production. Deployment target `3ea7ee8`; verified `HEAD == origin/master == 3ea7ee8`. Service verification passed: `shadz.service` active and enabled, Uvicorn on `127.0.0.1:8000`, port 8000 listening, `/health` returned `{"status":"ok"}`, startup completed normally, no relevant startup traceback. The first health request immediately after restart briefly returned connection-refused because Uvicorn had not finished starting; a readiness retry ~2s later confirmed normal startup — deployment-check timing, not an application regression. Production live Admin Panel regression testing then passed for the complete UI3G scope: summary counts; search by client name / Telegram username / Telegram User ID / assigned slug; all status filters; combined search + filter; no-match state; Refresh; Regenerate Access Code; Deactivate; Reactivate (with the correct `Reactivate` button label); Unassign slug without deleting the underlying slug/content; Delete Bot Client without deleting the underlying slug/content; bulk selection/filter safety; UI3G-B activation-state visibility; UI3G-C identity/activity visibility; UI3G-C.1 timestamp behavior; authoritative state remaining correct after Refresh. User confirmed: "All live tests passed."

**Files changed across the phase:** `bot_admin.py` (UI3G-B/C/C.1 — additive read-only response-schema enrichment on `AssignedSlugOut` / `BotClientOut` served by the existing `GET /admin/bot/clients`, plus the `_derive_activation_state` / `_derive_last_activity` / `_as_utc` helpers and UTC field validators; no new route), `static/admin.html` (card badges + identity/activity rows; `_allBotClients` cache, `renderBotClients()` / `renderBotSummary()`; lifecycle confirmation copy + `_botActionBusy` guard; search/filter + summary markup), and focused tests `tests/test_bot_admin_slug_activation_ui3g_b.py`, `tests/test_bot_admin_identity_activity_ui3g_c.py`, `tests/test_bot_admin_utc_serialization_ui3g_c1.py`, `tests/test_bot_client_control_centre_ui3g_def.py`. No other files.

**Quality record:** the focused UI3G-D/E/F test file passes; the final focused regression set after the `Reactivate` label hotfix was **123 passed**; `git diff --check` clean; the final hotfix (`3ea7ee8`) changed a single `static/admin.html` string with no backend/API/schema/docs/test changes. Broader-suite runs during the phase excluded the pre-existing, unrelated `tests/test_safety_deadline_engine_s5.py` failures (which also fail on the pre-UI3G baseline and are outside UI3G scope) — those deselected tests are **not** counted as passed.

**Architectural outcome:** UI3G improved the existing Bot Client management surface without expanding SHADZ OS unnecessarily. Existing Bot Client backend lifecycle semantics remain intact; existing APIs were reused wherever possible; UI3G-D/F are client-side-first; no new search API; no new summary API; no database migration for D/E/F (or anywhere in the phase); no Telegram runtime redesign; no change to NFC / public redirect behavior; no Safety / Page / Media / Link Engine semantics changed by D/E/F. UI3G remains part of the modular SHADZ Admin Panel architecture.

**Production:** VPS synchronized to `3ea7ee8`; runtime and live tests passed. This closure documentation is a separate commit.

**Next:** UI3H (UX / Safety Polish) and UI3I (Regression & Production Closure) remain ON HOLD, not cancelled or abandoned.
...
**Locked schema — four new tables:**
- **`SafetyUser`** (`safety_users`) — `id`, `display_name`, `timezone`, `daily_deadline` (TIME), `early_reminder_minutes` (required, no default), `is_active` (default `True`), `nfc_token` (unique, indexed — physical-token lookup key), `created_at`, `updated_at`.
- **`SafetyCheckIn`** (`safety_check_ins`) — `id`, `user_id` (FK → `safety_users.id`), `checked_in_at`, `latitude`/`longitude`/`accuracy_m` (all nullable), `source`, `created_at`.
- **`SafetyAlert`** (`safety_alerts`) — `id`, `user_id` (FK → `safety_users.id`), `safety_date` (DATE), `alert_type` (`early_reminder`/`missed_checkin`, DB `CHECK` constraint `ck_safety_alerts_alert_type`), `status` (`open`/`resolved`, default `open`, DB `CHECK` constraint `ck_safety_alerts_status`), `triggered_at`, `telegram_message_id`, `resolved_at`, `resolved_checkin_id` (FK → `safety_check_ins.id`), `created_at`.
- **`SafetyEmergency`** (`safety_emergencies`) — `id`, `user_id` (FK → `safety_users.id`), `triggered_at`, `latitude`/`longitude`/`accuracy_m` (all nullable), `status` (`open`/`acknowledged`/`resolved`, default `open`, DB `CHECK` constraint `ck_safety_emergencies_status`), `acknowledged_at`, `resolved_at`, `telegram_message_id`.

**Locked design facts:**
- `SafetyUser` is a fully independent identity — **not** another `BotClient`, and has no relationship to `BotClient`/`RedirectLink` in this phase. Normal Redirect Engine clients remain entirely unrelated to Safety Engine.
- Browser location will be required for every Safety Engine participant once the relevant runtime phase is implemented — S1 does **not** implement browser-location capture. The nullable `latitude`/`longitude`/`accuracy_m` columns on `safety_check_ins`/`safety_emergencies` exist now purely as foundation shape for that future phase.
- No ORM `relationship()` declarations were added, consistent with the rest of the codebase (`ActivationRecord`, `PageSlugAttachment`, `BotClientSlug`, `SlugMedia`) — all cross-table access is explicit `db.query(...)`.
- No `_run_migrations()`/`main.py` change was required — four net-new tables, handled entirely by `Base.metadata.create_all()`.

**Files changed:** `models.py` (new Safety Engine section: 4 models + 3 value-set constants), `tests/test_safety_engine_foundation.py` (new, 23 tests).

**Focused test result:** `tests/test_safety_engine_foundation.py` — 23/23 passed (table/column existence, FK definitions, `is_active`/`status` defaults, `nfc_token` uniqueness, allowed/rejected `alert_type` and `status` values for both `SafetyAlert` and `SafetyEmergency`, nullable location fields, `daily_deadline` time handling, required `early_reminder_minutes`).

**Full regression:** 684/684 passed. `git diff --check`: clean.
...
**Status:** Implementation and post-closure repair are complete. Final re-closure is pending documentation commit and VPS synchronization — do not treat UI3E as fully closed until that sync happens. Phone/mobile regression is intentionally deferred to the later Admin UI v0.3 phone-regression stage — desktop production live testing passed (pre-repair state, `9442e62`); mobile was not attempted and is not claimed as passed.

**Scope:** Media Manager improvements to `static/admin.html` and its supporting `media_admin.py` routes across five subphases (UI3E-A, UI3E-B, UI3E-C, UI3E-D1–D3, UI3E-E), plus a post-closure frontend cache-safety repair. No database schema changes, no migrations, no Restore capability, no permanent R2 object deletion.

**Final runtime code commit:** `d744ee524976e3a83ebabd139d26368a49380bf8` — fix(admin): clear stale media asset cache on load failure (post-closure repair; supersedes `9442e62` as the true final UI3E runtime code state)
...
- **UI3E-E — Media Admin Endpoint Test Coverage** (`9442e62`) — historical UI3E-E commit; production-code-zero: new consolidated `tests/test_media_admin_endpoints_ui3e_e.py` locking listing/`include_deleted`/usage-count, upload-URL presign + filename sanitization, upload completion, attach/replace/detach (including deleted-asset-attach rejection), soft-delete safety (zero-usage success, active-usage refusal, missing/already-deleted), and display-name update. R2 is stubbed at the existing `_get_r2_client` boundary — no live Cloudflare dependency. Did not duplicate the UI3E-D2 usage-endpoint tests. At the time of this commit `9442e62` was treated as the final UI3E runtime commit and closed via docs commit `9e35df4` — since superseded by the repair below.

**Post-closure independent raw-diff review repair (`d744ee5`):** Before final VPS closure sync, an independent full actual-Git-diff review of the entire completed UI3E phase (baseline → `9e35df4`) was performed and found one real frontend stale-cache bug in `loadStorage()` (`static/admin.html`): the non-2xx, empty-response, and network-error paths did not consistently clear both `_allAssets` and `_assetMap` — in particular the `catch` block cleared neither. Failure sequence this allowed: a successful load populates both caches → a later Refresh hits a network error → the UI correctly shows "Network error" → but a subsequent search/filter interaction could still re-render the old, stale asset cards from the untouched cache. Fix (`d744ee524976e3a83ebabd139d26368a49380bf8`): all three unusable-load branches (non-2xx, empty, catch) now clear both `_allAssets = []` and `_assetMap = {}` before returning/displaying the error state; successful-load behavior is unchanged; no backend change; no Media API change; no database/schema change; no Removed Assets behavior change.

**Regression strengthened alongside the repair:**
- **UI3E-C** (`tests/test_admin_media_search_filter_ui3e_c.py`) — cache-clearing is now locked per branch (non-2xx, empty response, network-error catch each individually asserted), plus the successful-load cache-population/render path.
- **UI3E-D2 backend** (`tests/test_media_asset_usage_ui3e_d2.py`) — multi-slug ordering is now asserted against the exact expected alphabetical order (not sorted in the assertion, so `.order_by(...)` is actually locked); added a duplicate-active-row test confirming `.distinct()` collapses two active `SlugMedia` rows for the same slug to one entry.
- **UI3E-D2 frontend** (`tests/test_admin_media_delete_warning_ui3e_d2.py`) — replaced a brittle fixed-count `return;` assertion with semantic branch-specific checks: non-2xx preflight failure, network preflight failure, active-usage block, user cancellation, and DELETE-call ordering relative to all four safety gates.
...
**Final regression (post-repair):** UI3E-C/D2(backend+frontend)/D3/E + Admin shell/dashboard + `test_page_admin` — **108/108 passed**. `git diff --check`: clean. (Earlier pre-repair closure regression was 100/100, at commit `9442e62`/`9e35df4`.)

**Runtime stale-cache smoke test (local, post-repair):** successful asset load populated both `_allAssets`/`_assetMap`; the next `/admin/media/assets` fetch was forced to fail; Storage Manager correctly displayed the network-error state and both caches were confirmed empty; a subsequent search/filter interaction did not re-render any stale asset card; a following successful load repopulated both caches normally. This was a local smoke test only — `d744ee5` has not yet been production-verified.

**Production live test:** Desktop production live testing passed for UI3E-C, UI3E-D2, and UI3E-D3 at their respective pre-repair closures. `d744ee5` has **not** been production live-tested yet. Phone/mobile regression remains intentionally deferred to the later Admin UI v0.3 phone-regression stage — not attempted, not claimed as passed.

**Production sync:** VPS was previously confirmed synchronized to `9442e62` (`HEAD == origin/master == 9442e62`, `shadz.service` active, `/health` 200) before the independent diff audit. The repair commit `d744ee5` is now on `origin/master` but **final VPS synchronization to the eventual re-closure commit is still pending** — do not treat VPS as synced to `d744ee5`.
...
**Conclusion:** no concrete regression gap found in automated coverage. Existing A1–A5 tests already satisfy the Phase A6 automated-test checklist. One explicit regression assertion was later added (see Token-consumption invariant below) to lock in a design point confirmed during production verification; no other test or runtime change was needed.

**Automated test results:**
- Focused: `tests/test_activation_engine_phase_a5.py` — `23 passed`
- Full suite: `350 passed, 28 subtests passed`
- `git diff --check`: clean (no runtime files modified at any point in this phase)

**Production preflight (VPS, Mr.Zack):** VPS synced to runtime commit `e7840d1`; `shadz.service` remained running throughout (no restart required — A6 made no runtime change); local `/health` returned 200; public `/health` returned 200; `/admin` without credentials returned 401; SQLite online backup taken and integrity-checked (`PRAGMA integrity_check` → `ok`), stored at `/opt/shadz-os/backups/shadz-a6-20260726224121.db`.

**Production live test — URL activation (disposable slug `url-mcyrdj`):** unactivated pre-state confirmed, `ActivationRecord` created with a token present; first public scan redirected to the Telegram activation deep link; "Activate Now" resolved the existing `BotClient` (no duplicate created); destination URL setup and Confirm succeeded, persisting `destination_url = https://google.com`; `activation_status` became `activated`, `owner_client_id = 3`, `activated_at` populated, exactly one `BotClientSlug` assignment created; public runtime resumed with a normal 302 to the confirmed destination. Replay verification: the original "Activate Now" callback and the original deep link were both rejected after activation, with no second activation. Archived-slug verification: archiving the slug produced the existing 410 expired page.

**Production live test — media activation (disposable slug `media-mfyzux`):** pre-Confirm state confirmed zero persistence — `activation_status` remained `unactivated`, no owner, no `BotClientSlug`, no `SlugMedia` row, `media_assets` count unchanged at 17. Post-Confirm: `activation_status` became `activated`, `owner_client_id = 3`, `activated_at` populated, exactly one `BotClientSlug` assignment, exactly one active `SlugMedia` row, and a new `MediaAsset` (`id = 18`, `storage_key = media/image/1785138624_telegram_image.jpg`, `original_filename = telegram_image.jpg`, `mime_type = image/jpeg`, `file_size = 226898`); `media_assets` count moved 17 → 18. The public media page returned 200 and rendered the uploaded image. Replay verification: the old Confirm and old "Activate Now" callback were both rejected, creating no additional `MediaAsset`/`SlugMedia` rows. Archived-slug verification: archiving the slug produced HTTP 410 and the existing expired page. `MediaAsset` id `18` and its R2 object were retained and not deleted during Phase A6 closure — it belongs to the (now archived) disposable test slug and must remain untouched. It is not added to the previously confirmed orphan cleanup-candidate list (`MediaAsset` ids `2` and `13`, see Phase A5) unless future evidence proves it is actually orphaned. Any future deletion of any `MediaAsset`/R2 object must follow the established verified orphan-cleanup workflow: database as source of truth; confirmation of no `SlugMedia` references at all — neither active nor historical/inactive; confirmation of the exact `MediaAsset.storage_key`; a dry-run/listing pass before any deletion; deletion of the R2 object only after every verification step above passes; deletion of the orphan `MediaAsset` DB row only after the R2 object has been removed; and a post-cleanup consistency check.

**Token-consumption invariant (confirmed in production, then locked in by tests):** `activation_token` remains stored, unchanged, after activation on both the URL and media test slugs; `activation_status` is what transitions from `unactivated` to `activated`; replay protection depends entirely on `activation_status != "unactivated"`, not on token removal — token consumption is status-based, not physical deletion. This is now covered by explicit assertions in `tests/test_activation_engine_phase_a5.py` (`test_successful_url_finalization_persists_destination_and_activates`, `test_successful_media_finalization_persists_asset_activates_and_assigns`), each capturing the token before finalization and asserting it is unchanged afterward.
...
**Activation Engine v1 final phase list:** A1 (data foundation) → A2 (first-scan Telegram entry + provisioning correction) → A3 (Bot Client creation/access code) → A4U (URL content setup) → A4M (media content setup) → A5 (atomic finalization) → A6 (regression audit, live testing, production closure, this entry). **All six phases are complete and production-verified — Phases A1–A5 were each individually deployed to production at their own closures. Activation Engine v1 is fully closed.**

**Next roadmap phase:** none — Activation Engine v1 has no further planned phases. Deferred/unscoped backlog (Admin UI activation-status visibility, analytics/dashboard upgrades, activation support for new slug types, lifecycle redesign, legacy-row backfill, `content_type`-conversion provisioning, R2 orphan cleanup, `delete_activation_lifecycle_for_slug` wiring) remains outside this closure and is not a committed phase.
